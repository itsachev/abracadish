...
export default function AddToShoppingListButton({ recipe, className = "" }) {
  ...
  return (
    <button
      ...
      className={`gradient-accent-soft flex w-full items-center justify-center gap-2 rounded-3xl border border-accent/30 py-3.5 text-center text-sm font-semibold text-foreground transition-transform active:scale-[0.98] disabled:opacity-60 ${className}`}
    >
