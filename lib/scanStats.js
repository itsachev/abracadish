...
// [{ cuisine, count, firstScannedAt, dishNames }], most-recently-discovered
// first — the data behind the Dish Passport's stamp grid. Same "Unknown"
// exclusion as computeCuisineBadges.
export function computeCuisinePassport(scans) {
  const byCuisine = new Map();
  for (const scan of scans) {
    const cuisine = scan.cuisine?.trim();
    if (!cuisine || cuisine.toLowerCase() === "unknown") continue;

    const entry = byCuisine.get(cuisine) ?? { cuisine, count: 0, firstScannedAt: scan.createdAt, dishNames: [] };
    entry.count += 1;
    if (new Date(scan.createdAt) < new Date(entry.firstScannedAt)) {
      entry.firstScannedAt = scan.createdAt;
    }
    if (scan.dishName && !entry.dishNames.includes(scan.dishName)) {
      entry.dishNames.push(scan.dishName);
    }
    byCuisine.set(cuisine, entry);
  }

  return [...byCuisine.values()].sort(
    (a, b) => new Date(b.firstScannedAt) - new Date(a.firstScannedAt)
  );
}
